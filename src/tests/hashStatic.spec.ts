...
import { hashStatic, hashStaticCli } from "../";
...
describe("hashStatic", () => {
  ...
  afterEach(() => {
    jest.clearAllMocks();
    sh.rm("-rf", "src/tests/fixtures/basic/processed");
  });
  ...
  test("hashStatic – matches snapshot (minified)", async () => {
    sh.cp(
      "-r",
      "src/tests/fixtures/basic/original",
      "src/tests/fixtures/basic/processed"
    );

    const entry = "src/tests/fixtures/basic/processed/index.html";
